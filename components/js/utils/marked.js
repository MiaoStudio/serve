...
Lexer.prototype.token = function(src, top, bq) {
  ...
  while (src) {
    // private
    if (cap = this.rules.private.exec(src)) {
...
function emojiDefaultTemplate(emoji) {
  ...
  return '<span class="emoji emoji-' + encodeURIComponent(emoji) + '" data-src="graphics/emojis/'+ encodeURIComponent(emoji) + '.png" align="absmiddle" title="' + escape(emoji) + '"></span>';
}
...
Renderer.prototype.heading = function(text, level, raw) {
  ...
  /*
   * 这里是自己家的，为了生成唯一id
   * this.options.allheadings 每次进入函数都在函数内生成一个变量[]，用来储存所有的 标题类
  */
  var escapedText =  this.options.headerPrefix + raw.toLowerCase().replace(/[\s\r\n]+/g, '-');
  var duplicateIndex = this.options.allheadings.map(function (h) { return h.text }).indexOf(escapedText);
  var duplicateText
  if (duplicateIndex === -1) {
    this.options.allheadings.push({
      text: escapedText,
      count: 0
    })
  } else {
    this.options.allheadings[duplicateIndex].count++
    duplicateText = escapedText + '-' + this.options.allheadings[duplicateIndex].count
  }
  return '<h' + level + ' id="' + (duplicateText || escapedText) + '">' +
            '<a class="anchor" href="#' + (duplicateText || escapedText) + '"><span class="octicon octicon-link"></span></a>' +
            text +
         '</h' + level + '>\n';
};
...
Renderer.prototype.formula = function(expr) {
  if (expr[0] === '$' && expr[expr.length - 1] === '$') {
    ...
    try {
      var html = katex.renderToString(expr, null, { displayMode: true });
      if (displayStyle) {
        html = html.replace(/class=\"katex\"/g, 'class="katex katex-block" style="display: block;text-align: center;"');
      }
      return html;
    } catch (e) {
      console.error(e);
...
Renderer.prototype.image = function(href, title, text) {
  ...
  if (title) {
    ...
    if (textData) {
      if (textData[1]) {
        out += ' class="align-' + textData[1].trim() + '"';
      } else {
        out += ' class="align-none"';
      }

      if (textData[2]) {
        var whData = textData[2].trim().split('x');
        if (whData[0]) {
          out += ' width="' + whData[0].trim() + '"';
        }
      }
    }
    out += ' >';
  ...
  if (title) {
    out += ' title="' + title + '"';
    out += this.options.xhtml ? '/><figcaption>' + title + '</figcaption></figure>' : '><figcaption>' + title + '</figcaption></figure>';
  } else {
...
Renderer.prototype.tocItem = function (id, level, text) {
  ...
  return '<li class="toc-item toc-level-' + level + '"><a class="toc-link" href="#tide-' + id + '"><span class="toc-number"></span> <span class="toc-text">' + text + '</span></a></li>';
};
...
Renderer.prototype.private = function(text) {
...
Parser.prototype.tok = function() {
  ...
  switch (this.token.type) {
    ...
    case 'private_start': {
      var body = '';
      while (this.next().type !== 'private_end') {
        body += this.tok();
      }
      return this.renderer.private(body);
    }
    case 'list_start': {
      var body = ''
        , taskList = false
